	...
	public static void main(String[] args) {
		...
		*/
		Function<String, Boolean> filter = k -> {
			...
			return tokens[5].equals("-1") == false
		...
		JavaDoubleRDD nbPatternsDouble = distFile.filter(filter).mapToDouble(k -> Double.valueOf(k.split(";")[4]));

		// Convertit le JavaDoubleRDD en JavaRDD<Double> pour pouvoir utiliser la fonction getPercentiles
		JavaRDD<Double> nbPatternsRDDDouble = nbPatternsDouble.map(k -> k);
		...
		*/
		StatCounter sc = nbPatternsDouble.stats();
		double[] diffPercentiles = {0.25,0.5,0.75};
		double[] percentiles = getPercentiles(nbPatternsRDDDouble, diffPercentiles, nbPatternsDouble.count(), nbPatternsDouble.getNumPartitions());
		double mean = sc.mean();
		double min = sc.min();
		double max = sc.max();
		Tuple2<double[],long[]> histo = nbPatternsDouble.histogram(5);
